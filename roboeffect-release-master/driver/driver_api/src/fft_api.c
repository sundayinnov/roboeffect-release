...
#define BFLOOR_Q1d31(x)                         bfloor((x), 1, 31)
#define BFLOOR_Q1d15(x)                         bfloor((x), 1, 15)
...
static const int32_t COS1024_TAB[] =
{
...
static inline int32_t bfloor(int64_t x, int32_t intbits, int32_t fracbits)
{
...
int32_t cfft_sw(uint32_t Type, int32_t* DataInOut, uint32_t DataCount)
{
    ...
    {
        // j is i's bit reversal.
        // e.g. if DataCount = 64, M = 6, when i = 3(000011), j = 48(110000), only swap when j > i
        k = DataCount / 2;
