...
void DeviceStorRead10(void)
{
	...
	GPIO_RegOneBitClear(GPIO_A_OUT,GPIO_INDEX14);
	GPIO_RegOneBitSet(GPIO_A_OUT,GPIO_INDEX14);
	GPIO_RegOneBitClear(GPIO_A_OUT,GPIO_INDEX14);
	////ReqLBA *= SD_BLOCK_SIZE;
	*(uint32_t *)0x4000202C |= 0x20;
	SDIO_RXStart(1,(i%2)?CARD_BUF_A:CARD_BUF_B,SD_BLOCK_SIZE);
	SDIO_CmdSend(1,CMD18_READ_MULTIPLE_BLOCK, ReqLBA, 20);
	SDIO1_ClkEnable();
	while(!DMA_InterruptFlagGet(PERIPHERAL_ID_SDIO1_RX, DMA_DONE_INT));
	...
	SDIO_CmdSend(1,CMD12_STOP_TRANSMISSION, 0, 20);
	SDIO1_ClkDisable();
	DeviceStorSendCSW(0);
}
...
void DeviceStorWrite10(void)
{
	...
	while(SDIO_IsDataLineBusy(1));
	SDIO_CmdSend(1,CMD25_WRITE_MULTIPLE_BLOCK, ReqLBA, 20);
	for(i=1;i<ReqCnt;i++)
	{
		SDIO_TXStart(1,(i%2)?CARD_BUF_B:CARD_BUF_A,SD_BLOCK_SIZE);
		if(OTG_DeviceBulkReceive(DEVICE_BULK_OUT_EP, (i%2)?CARD_BUF_A:CARD_BUF_B, 512,&DataLength,10000) != DEVICE_NONE_ERR)
		{
			OTG_DBG("USB ERROR\n");
			return;
		}
		while(!SDIO_TXIsDone(1));
		SDIO_TXDoneFlagClear(1);
		while(SDIO_IsDataLineBusy(1));
	}
	SDIO_TXStart(1,(i%2)?CARD_BUF_B:CARD_BUF_A,SD_BLOCK_SIZE);
	while(!SDIO_TXIsDone(1));
