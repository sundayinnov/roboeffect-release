...
#define	DEVICE_FS_ISO_IN_MPS		256//only use by ep4 5
#define	DEVICE_FS_ISO_OUT_MPS		1024//only use by ep4 5
 ...
 * @param  NONE
 * @return NONE
 * @note 调用后，DP上拉影响Host检测， 必须配以OTG_PortDeviceIsLink检测。
 */
...
/**
 * @brief  DEVICE模式下使能某个总线事件中断
 * @param  BusEvent 总线事件号
 * @param  Func 中断回调函数指针
 * @return NONE
 */
void OTG_BusInterruptEnable(uint8_t BusEvent, FPCALLBACK Func);


/**
 * @brief  DEVICE模式下禁止某个总线事件中断
 * @param  BusEvent 总线事件号
 * @return NONE
 */
void OTG_BusInterruptDisable(uint8_t BusEvent);
